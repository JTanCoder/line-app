<template name="fuled-picker">
	<view class="fuled-picker" :key="createKey" :data-key="createKey">
		<view class="mask" :class="{'visible':visible}" @tap="onCancel" @touchmove.stop.prevent catchtouchmove="true"></view>
		<view class="fuled-picker-cnt" :class="{'visible':visible}">
			<view class="fuled-picker-header"  @touchmove.stop.prevent catchtouchmove="true">
				<text class="fuled-picker-cancel-but" @tap.stop.prevent="onCancel">取消</text>
				<slot></slot>
				<text :style="{'color':themeColor}" @tap.stop.prevent="pickerConfirm">确定</text>
			</view>
			<date-picker 
				v-if="mode=='date'" 
				class="fuled-picker-wrapper"
				:startYear="startYear"
				:endYear="endYear"
				:value="value"
				:fields="fields"
