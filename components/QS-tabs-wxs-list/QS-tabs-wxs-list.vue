	...
	<view  class="container" :style="{'z-index': zIndex }">
		<view  class="tabs-container" :style="{
			...
			<scroll-view  id="tabs-scroll" scroll-x  :scroll-left="scrollLeft" class="tabs-scroll"  scroll-with-animation>
				<view  class="tabs-scroll-box" :style="{ 'height': tabHeight, 'line-height': tabHeight }">
					<view  class="tabs-scroll-item" :style="{ 'min-width': minWidth, 'padding': '0 ' + space }"
					v-for="(item, index) in tabs" 
					:id="preId + index"
					:key="index"
					@tap="tabTap(index)">
						<view class="hide_text"  :style="{ 'font-size': fontSize }">
							{{item.name || item}}
						</view>
			...
			<block v-if="String(hasRefresh) === 'true'">
				...
				:refreshstatus="refreshStatus">
					...
					:style="{ 
						...
						mode="scaleToFill" 
						class="refresh-image"
						:class="{'isRefreshing': isRefreshing}"
						></image>
	...
	export default {
		...
		computed: {
			...
			getSwiperHieght() {
				...
				//console.log(Number(this.height) - Number(this.tabsHeight))
		...
		methods: {
			...
			setRefreshShow(obj) {
				console.log('设置refreshShow' + JSON.stringify(obj));
				// #ifndef H5
