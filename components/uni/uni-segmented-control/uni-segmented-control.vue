<template>
	<view :class="[styleType === 'text'?'segmented-control--text' : 'segmented-control--button' ]" :style="{ borderColor: styleType === 'text' ? '' : activeColor }" class="segmented-control">
        ...
        backgroundColor: index === currentIndex && styleType === 'button' ? activeColor : '',borderColor: index === currentIndex&&styleType === 'text'||styleType === 'button'?activeColor:'transparent'
			...
			<view :class="[index != values.length - 1?'segmented-control-item-view-right-line': '','segmented-control-item-view','mb-1']">
				<text :style="{color:
				index === currentIndex
				  ? styleType === 'text'
				    ? activeColor
...
<style scoped>
	...
	.segmented-control-item-view-right-line{
		border-right: solid 1px #64A8D1;
	}
	
	.segmented-control-item-view{
		width: 100%;
		text-align: center;
		line-height: 25px;
		font-size: 12rpx;
